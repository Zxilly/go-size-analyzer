...
func (s *wordTracker) pair(ptr, length trackedWord) {
	...
	switch {
	case s.constantPointers && ptr.kind == wordConstant && length.kind == wordConstant:
		if length.value > 0 {
...
func (s *wordTracker) store(base int, offset int64, w trackedWord) {
	...
	// Unknown object bases may alias. Retain only stores through the same
	// base, rather than joining evidence across intervening aliasing writes.
	for i := range s.stores {
		if s.stores[i].base != base {
			s.stores[i].word = trackedWord{}
		}
	}
	w.age = s.clock

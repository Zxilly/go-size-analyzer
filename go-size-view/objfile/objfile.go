...
type RelocStringer interface {
	// String insnOffset is the offset of the instruction containing the relocation
	// from the start of the symbol containing the relocation.
	String(insnOffset uint64) string
}

var openers = []func(any) (rawFile, error){
	openElf,
	openMacho,
	openPE,
}

// Create Open opens the named file.
// The caller must call f.Close when the file is no longer needed.
func Create(file *os.File, r any) (*File, error) {
	for _, try := range openers {
		if raw, err := try(r); err == nil {
			return &File{file, []*Entry{{raw: raw}}}, nil
		}
	}
	return nil, fmt.Errorf("unrecognized object file")
}
